    ...
    if (m.isGroup) {
        if (m.mentionedJid.includes(this.user.jid)) {
            await this.send2Button(m.chat,
                isBanned ? 'Nandhutty not active' : banned ? 'you are banned' : '𝐌𝐀𝐆𝐈𝐂𝐈𝐀𝐍 active',
                '𝐌𝐀𝐆𝐈𝐂𝐈𝐀𝐍',
...
`.trim(), '𝐌𝐀𝐆𝐈𝐂𝐈𝐀𝐍', 'Git', ',git', m)
    ...
    if (set.autoupdatestatus) {
        if (new Date() * 1 - set.status > 1000) {
            ...
            await this.setStatus(`Active during ${uptime} | Mode: ${set.self ? 'Private' : set.group ? 'Group Only' : 'Publik'} | 𝐌𝐀𝐆𝐈𝐂𝐈𝐀𝐍 by 𝙍𝘼𝙃𝙀𝙀𝙎`).catch(_ => _)
